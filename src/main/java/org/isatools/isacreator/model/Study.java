...
public class Study extends ISASection {
  ...
  /**
   * A Study is the all-encompassing object which groups together an assay or set of Assays with the
   * aim of answering some biological quaestion.
   *
   ...
   */
  public Study(String studyId, String studyTitle, String dateOfSubmission, String publicReleaseDate,
      String studyDesc, String studySample) {
    super();
  ...
  /**
   * Return a List (@see java.util.List<Publication> ) of Publications contained within this Study
   *
  ...
  /**
   * Return a List (@see java.util.List<StudyDesign> ) of StudyDesign objects contained within this
   * Study
   *
  ...
  /**
   * Remove a Publication by providing the PubMedId or the PublicationTitle of the Publication. One
   * of the parameters needs to have data in it, otherwise this method would be useless.
   *
   * @param pubMedId - the PubMed ID for the publication to be removed. Can be left as an empty
   *        string if there is no PubMedID
   * @param publicationTitle - the Publication Title for the publication to be removed. Can be left
   *        as an empty String if not known.
   */
  public void removePublication(String pubMedId, String publicationTitle) {
    Publication toRemove = null;

    for (Publication pub : publications) {
      if (pub.getPublicationTitle().equals(publicationTitle)
          || pub.getPubmedId().equals(pubMedId)) {
        toRemove = pub;
        break;
      }
    }
   ...
   *
   * @return Map<String, Assay> where the String is the AssayReference. Map used to make lookups
   *         quicker.
   */
  ...
  // ((AssaySpreadsheet)
  // ApplicationManager.getUserInterfaceForISASection(assays.get(key))).getSpreadsheet()
  // .getSpreadsheetFunctions().removeColumnByName("Factor Value[" + factorName + "]");
  // }
  //
  // ((AssaySpreadsheet)
  // ApplicationManager.getUserInterfaceForISASection(studySampleRecord)).getSpreadsheet()
  // .getSpreadsheetFunctions().removeColumnByName("Factor Value[" + factorName + "]");
  //
  ...
  public void removeContact(String forename, String surname, String emailAddress) {
    ...
    for (Contact contact : contacts) {
      if (contact.getFirstName().equals(forename) && contact.getLastName().equals(surname)
   ...
   *
   * @return Object[][] containing all the data contained in the Study Sample. Top Most row
   *         (Object[0] will contain all the headers for the Study Sample.)
   */
  ...
  private boolean checkContactExists(String forename, String surname, String email) {
    for (Contact c : contacts) {
      if (c.getFirstName().equals(forename) && c.getLastName().equals(surname)
