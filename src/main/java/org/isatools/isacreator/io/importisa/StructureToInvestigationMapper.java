 ...
 * 
 * License: ISAcreator is licensed under the Common Public Attribution License version 1.0 (CPAL)
 * 
 * EXHIBIT A. CPAL version 1.0 The contents of this file are subject to the CPAL version 1.0 (the
 * License); you may not use this file except in compliance with the License. You may obtain a copy
 * of the License at http://isa-tools.org/licenses/ISAcreator-license.html. The License is based on
 * the Mozilla Public License version 1.1 but Sections 14 and 15 have been added to cover use of
 ...
 * 
 * The Original Code is ISAcreator. The Original Developer is the Initial Developer. The Initial
 * Developer of the Original Code is the ISA Team (Eamonn Maguire, eamonnmag@gmail.com; Philippe
 * Rocca-Serra, proccaserra@gmail.com; Susanna-Assunta Sansone, sa.sanson@gmail.com;
 * http://www.isa-tools.org). All portions of the code written by the ISA Team are Copyright (c)
 * 2007-2011 ISA Team. All Rights Reserved.
 * 
 * EXHIBIT B. Attribution Information Attribution Copyright Notice: Copyright (c) 2008-2011 ISA Team
 * Attribution Phrase: Developed by the ISA Team Attribution URL: http://www.isa-tools.org Graphic
 * Image provided in the Covered Code as file:
 * http://isa-tools.org/licenses/icons/poweredByISAtools.png Display of Attribution Information is
 * required in Larger Works which are defined in the CPAL as a work which combines Covered Code or
 * portions thereof with code not governed by the terms of the CPAL.
 * 
 * Sponsors: The ISA Team and the ISA software suite have been funded by the EU Carcinogenomics
 * project (http://www.carcinogenomics.eu), the UK BBSRC (http://www.bbsrc.ac.uk), the UK NERC-NEBC
...
/**
 * Maps the DataStructure created by @see InvestigationImport to an Investigation object, with the
 * relevant Study data contained as well.
 *
...
public class StructureToInvestigationMapper {
  ...
  public Pair<Boolean, Investigation> createInvestigationFromDataStructure(
    ...
    return new MutablePair<Boolean, Investigation>(validateInvestigationFile(investigation),
        investigation);
  }
  ...
  private Investigation processInvestigation(
    ...
    OrderedMap<InvestigationFileSection, Set<String>> sectionFields =
        new ListOrderedMap<InvestigationFileSection, Set<String>>();
    ...
    for (InvestigationFileSection investigationSection : investigationSections.keySet()) {
      if (investigationSection == InvestigationFileSection.INVESTIGATION_SECTION) {
        ...
        Pair<Set<String>, Investigation> processedInvestigationSection =
        ...
        Pair<Set<String>, List<OntologySourceRefObject>> processedOntologySection =
            processOntologySourceReferences(investigationSections.get(investigationSection));
        sectionFields.put(investigationSection, processedOntologySection.getLeft());
        ontologySources = new HashSet<OntologySourceRefObject>(processedOntologySection.getRight());
        ...
        sectionFields.put(investigationSection, processedPublicationSection.getLeft());
        publications = processedPublicationSection.getRight();

      } else if (investigationSection == InvestigationFileSection.INVESTIGATION_CONTACTS_SECTION) {
        Pair<Set<String>, List<Contact>> processedContactSection =
            processContacts(investigationSection, investigationSections.get(investigationSection));
        sectionFields.put(investigationSection, processedContactSection.getLeft());
        contacts = processedContactSection.getRight();
      }
  ...
  private Pair<Set<String>, Investigation> processInvestigationSection(
      OrderedMap<String, List<String>> investigationSection) {
    ...
    for (int hashCode : ontologyFields.keySet()) {
      ...
      try {
        String value = groupElements(ontologyField.get(IOUtils.TERM),
            record.get(ontologyField.get(IOUtils.TERM)),
            record.get(ontologyField.get(IOUtils.ACCESSION)),
            record.get(ontologyField.get(IOUtils.SOURCE_REF)));
    ...
    return new MutablePair<Set<String>, Investigation>(sectionFields, investigation);
  }
  ...
  private Study processStudy(
    ...
    for (InvestigationFileSection studySection : studySections.keySet()) {
      if (studySection == InvestigationFileSection.STUDY_SECTION) {
        ...
        Pair<Set<String>, Study> processedStudySection =
            processStudySection(studySections.get(studySection));
        study = processedStudySection.getRight();
        sectionFields.put(studySection, processedStudySection.getLeft());

      } else if (studySection == InvestigationFileSection.STUDY_FACTORS) {

        Pair<Set<String>, List<Factor>> processedFactorsSection =
            processFactors(studySections.get(studySection));
        sectionFields.put(studySection, processedFactorsSection.getLeft());
        factors = processedFactorsSection.getRight();

      } else if (studySection == InvestigationFileSection.STUDY_DESIGN_SECTION) {

        Pair<Set<String>, List<StudyDesign>> processedStudyDesignSection =
            processStudyDesigns(studySections.get(studySection));
        sectionFields.put(studySection, processedStudyDesignSection.getLeft());

        studyDesigns = processedStudyDesignSection.getRight();

      } else if (studySection == InvestigationFileSection.STUDY_ASSAYS) {

        Pair<Set<String>, List<Assay>> processedAssaySection =
            processAssay(studySections.get(studySection));
        sectionFields.put(studySection, processedAssaySection.getLeft());
        assays = processedAssaySection.getRight();

      } else if (studySection == InvestigationFileSection.STUDY_PUBLICATIONS) {

        Pair<Set<String>, List<Publication>> processedPublicationSection =
            processPublication(studySection, studySections.get(studySection));
        sectionFields.put(studySection, processedPublicationSection.getLeft());
        publications = processedPublicationSection.getRight();

      } else if (studySection == InvestigationFileSection.STUDY_PROTOCOLS) {

        Pair<Set<String>, List<Protocol>> processedProtocolSection =
            processProtocol(studySections.get(studySection));
        sectionFields.put(studySection, processedProtocolSection.getLeft());
        protocols = processedProtocolSection.getRight();

      } else if (studySection == InvestigationFileSection.STUDY_CONTACTS) {

        Pair<Set<String>, List<Contact>> processedContactSection =
            processContacts(studySection, studySections.get(studySection));
        sectionFields.put(studySection, processedContactSection.getLeft());
        contacts = processedContactSection.getRight();
      }
    ...
    if (study != null) {
      ...
      // we want to add new fields, but want to keep the general working of the configuration. So we
      // do just that.
      // new fields (unknown) will be treated as Strings, other known values will acquire the
      // properties
  ...
  private Pair<Set<String>, Study> processStudySection(
      OrderedMap<String, List<String>> studySection) {
    ...
    return new MutablePair<Set<String>, Study>(sectionFields, study);
  }

  private Pair<Set<String>, List<OntologySourceRefObject>> processOntologySourceReferences(
      OrderedMap<String, List<String>> ontologySection) {
    ...
    return new MutablePair<Set<String>, List<OntologySourceRefObject>>(sectionFields,
        ontologySources);
  }

  private Pair<Set<String>, List<Publication>> processPublication(InvestigationFileSection section,
      OrderedMap<String, List<String>> publicationSection) {
    List<Publication> publications = new ArrayList<Publication>();
    ...
    return new MutablePair<Set<String>, List<Publication>>(sectionFields, publications);
  }

  private Pair<Set<String>, List<StudyDesign>> processStudyDesigns(
      OrderedMap<String, List<String>> studyDesignSection) {
    List<StudyDesign> studyDesigns = new ArrayList<StudyDesign>();
    ...
    return new MutablePair<Set<String>, List<StudyDesign>>(sectionFields, studyDesigns);
  }

  private Pair<Set<String>, List<Contact>> processContacts(InvestigationFileSection section,
      OrderedMap<String, List<String>> contactSection) {
    List<Contact> contacts = new ArrayList<Contact>();
    ...
    return new MutablePair<Set<String>, List<Contact>>(sectionFields, contacts);
  }

  private Pair<Set<String>, List<Factor>> processFactors(
      OrderedMap<String, List<String>> factorSection) {
    List<Factor> factors = new ArrayList<Factor>();
    ...
    return new MutablePair<Set<String>, List<Factor>>(sectionFields, factors);
  }

  private Pair<Set<String>, List<Protocol>> processProtocol(
      OrderedMap<String, List<String>> protocolSection) {
    List<Protocol> protocols = new ArrayList<Protocol>();
    ...
    return new MutablePair<Set<String>, List<Protocol>>(sectionFields, protocols);
  }

  private Pair<Set<String>, List<Assay>> processAssay(
      OrderedMap<String, List<String>> assaySection) {
    ...
    return new MutablePair<Set<String>, List<Assay>>(sectionFields, assays);
  }
   ...
   */
  private String groupElements(String fieldBeingCombined, String term, String accession,
      String sourceRef) throws MalformedOntologyTermException {
    String toReturn = "";
    ...
    if (term.contains(";") && sourceRef.contains(";")) {
      ...
      if ((splitTerms.length == splitSourceRefs.length)) {
        ...
        // we know that everything is properly formed between the source refs, accessions, and the
        // terms.
        for (int i = 0; i < splitTerms.length; i++) {
          if (splitSourceRefs[i].equals("") || splitSourceRefs[i].equals(" ")) {
            ...
            if ((splitAccession != null) && (i < splitAccession.length)) {
              accToAdd = splitAccession[i];
            }

            ontologyTermsDefined.add(new OntologyTerm(splitTerms[i], accToAdd, null,
                getOntologySource(splitSourceRefs[i])));
      ...
      if (sourceRef.equals("")) {
        ...
        if (!term.trim().equals("") && !sourceRef.trim().equals("")) {

          if (accession.contains("http://"))
            ontologyTermsDefined
                .add(new OntologyTerm(term, accession, accession, getOntologySource(sourceRef)));
          else {
            ...
            if (!(ISAcreatorProperties.getOntologyTermURIProperty()
                && ot.getOntologyTermURI() != null && !ot.getOntologyTermURI().equals("")))
              toReturn = term;
  ...
  private boolean validateInvestigationFile(Investigation investigation) {
    ...
    for (Study study : investigation.getStudies().values()) {
      if (studyNames.contains(study.getStudyId())) {
        String message = "Duplicate study names found in investigation! Study with with ID : "
            + study.getStudyId() + " already exists!";
      ...
      for (Assay assay : study.getAssays().values()) {
        if (assayNames.contains(assay.getAssayReference())) {
          String message = "Duplicate assay found in investigation! Assay with with name : "
              + assay.getAssayReference() + " already exists!";
          messages.add(new ErrorMessage(ErrorLevel.ERROR, message));
          log.info(message);
          return false;
        } else {
    ...
    if (missingOntologyObjects.size() > 0) {
      ...
      messages.add(new ErrorMessage(ErrorLevel.ERROR,
          "Some ontology sources are not defined in the ONTOLOGY SOURCE REFERENCE section -> "
              + missing));
      log.info("Some ontology sources are not defined in the ONTOLOGY SOURCE REFERENCE section -> "
          + missing);
      return false;
    }
